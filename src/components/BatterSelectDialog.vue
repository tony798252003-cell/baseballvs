<template>
  <div v-if="show" class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
    <div class="bg-white border border-slate-100 rounded-3xl p-6 max-w-4xl w-full max-h-[85vh] overflow-hidden flex flex-col shadow-2xl">
      <div class="flex items-center justify-between mb-4">
        <h2 class="text-3xl font-black text-slate-900">選擇代打</h2>
      ...
      <div class="flex gap-2 mb-4 flex-wrap">
        ...
        <button v-if="otherTeamsWithBatters.length > 0"
          ...
          :class="[
            ...
            isOtherTeamSelected ? 'ring-2 ring-indigo-500 bg-indigo-50 border-indigo-300 scale-105' : 'bg-white text-slate-600 border-slate-200 hover:bg-indigo-50 hover:border-indigo-300'
          ]">
          {{ showOtherTeams ? '▲' : '▼' }}
        </button>
      ...
      <div class="flex-1 overflow-y-auto grid grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-2">
        <button v-for="player in filteredBatters" :key="player.name + player.number"
          ...
          class="bg-white hover:shadow-md disabled:bg-slate-50 disabled:opacity-50 p-2 rounded-xl transition-all duration-150 transform hover:scale-105 disabled:scale-100 text-left border border-indigo-100 hover:border-indigo-400 disabled:border-slate-100 shadow-sm cursor-pointer disabled:cursor-not-allowed">
          <div class="flex items-start gap-2">
            ...
            <div v-else class="w-12 h-12 rounded-full bg-indigo-50 flex items-center justify-center text-xl flex-shrink-0">⚾</div>
            <div class="flex-1 min-w-0 flex flex-col justify-between">
              ...
              <div class="text-xs text-indigo-500 font-bold">#{{ player.number }} · {{ player.mainPosition }}</div>
            </div>
          </div>
        </button>
      </div>
